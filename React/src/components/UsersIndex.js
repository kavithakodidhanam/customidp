...
const fetchUsersData = async (page, pageSize, pageFilter, pageSortBy) => {
    ...
    if (pageSortBy.length > 0) {
        ...
        paramStr = `${paramStr}&sortField=${sortParams.id}&sort=${sortyByDir}`
    }
    try {
    
        const response = await axios.get(
            `http://localhost:8087/query/getIdpuser?page=${page}&size=${pageSize}${paramStr}`, { headers: authHeader() }
      
      //`http://ec2-100-27-22-147.compute-1.amazonaws.com:8087/query/getIdpuser?page=${page}&size=${pageSize}${paramStr}`, { headers: authHeader() }
            );
